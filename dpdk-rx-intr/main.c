 ...
 * Hexdump control
 */
static bool do_hexdump = true;

/**
 * Port configuration, startup and cleanup
 */
static int
...
static void
turn_on_intr(uint16_t port_id, uint16_t queue_id)
{
	int ret;

	ret = rte_eth_dev_rx_intr_enable(port_id, rx_queue_id);
	if (ret < 0)
		fprintf(stderr, "%s: %s(), lcore %u, rte_eth_dev_rx_intr_enable() failed (%d): %s\n",
			APP_NAME, __func__, rte_lcore_id(), ret, rte_strerror(-ret));
}

static void
turn_off_intr(uint16_t port_id, uint16_t queue_id)
{
	int ret;

	ret = rte_eth_dev_rx_intr_disable(port_id, rx_queue_id);
	if (ret < 0)
		fprintf(stderr, "%s: %s(), lcore %u, rte_eth_dev_rx_intr_disable() failed (%d): %s\n",
			APP_NAME, __func__, rte_lcore_id(), ret, rte_strerror(-ret));
}
...
worker(__attribute__((unused)) void *dummy)
{
	...
	ret = rte_eth_dev_rx_intr_ctl_q(port_id, rx_queue_id, RTE_EPOLL_PER_THREAD,
					RTE_INTR_EVENT_ADD, 0);
	if (ret < 0) {
		fprintf(stderr, "%s: %s(), lcore %u, rte_eth_dev_rx_intr_ctl_q() failed (%d)\n",
			APP_NAME, __func__, rte_lcore_id(), ret);
		return 0;
	}
	...
	while (rte_atomic16_read(&is_running)) {
		struct rte_epoll_event event[4];
		int c;
		int n;
		int i;

		if (!intr_en) {
			turn_on_intr(port_id, rx_queue_id);
			intr_en = 1;
		}

		n = rte_epoll_wait(RTE_EPOLL_PER_THREAD, event, 4, 300);
		for (i = 0; i < n; ++i) {
			printf("%s: %s(), lcore %u, woken up\n",
				APP_NAME, __func__, rte_lcore_id());
			wake_ups++;
		}

		if (intr_en && n > 0) {
			turn_off_intr(port_id, rx_queue_id);
			intr_en = 0;
		}

		struct rte_mbuf *mbuf[32] = { 0 };
		uint16_t nb_rx = rte_eth_rx_burst(port_id, rx_queue_id, mbuf, 32);
		RTE_ASSERT(0 <= nb_rx && nb_rx <= 32);
		if (nb_rx > 0) {
			int j;
			...
			for (j = 0; j < nb_rx; ++j) {
				if (do_hexdump) {
					...
					uint32_t len = rte_pktmbuf_pkt_len(mbuf[j]);
					const uint8_t *data = rte_pktmbuf_read(mbuf[j], 0, len, buffer);
					printf("%s: %s() packets received, nb_rx=%u\n",
						APP_NAME, __func__, nb_rx);
	...
	printf("%s: %s() packets received = %llu, wake ups = %llu\n", APP_NAME, __func__, received, wake_ups);
	printf("%s: %s() ending\n", APP_NAME, __func__);
